...
enum state
{
  def,drone_detected,move_back,approach_again,b_offset,final_approach,final_turn,MoveForward,MoveForwardOA,mission_complete, mission_failed
};
enum GoalState
...
int main (int argc, char **argv)
{
  ...
  double K_Move = 1.0;
  ros::param::set("rover0/moveforward","done");
  ros::param::set("rover0/moveforwardOA","done");
  std::string moveforward = "done";std::string moveforwardOA = "done";
  while (nh.ok()) {
    try{
    ...
    if(ros::param::get("rover0/moveforward",moveforward) && moveforward.compare("start")==0)
    {
      S = MoveForward;
    }
    if(ros::param::get("rover0/moveforwardOA",moveforwardOA) && moveforwardOA.compare("start")==0)
    {
      S = MoveForwardOA;
    }
    switch (S) {
...
//        ROS_WARN("No wasp visibale Going back anyways");
    ...
    case final_approach:
    {
      ...
      // ROS_INFO("angle to wasp: %f",atan2(arm_wasp.getOrigin().getY(),arm_wasp.getOrigin().getX())*180/M_PI);
      approach_goal = rov + rov_b+ tf::Vector3(1-arm_wasp.getOrigin().getX(),0,0).rotate(tf::Vector3(0,0,1),tf::getYaw(rover_pose.orientation));
    ...
    case MoveForward:
      ...
      ROS_INFO("APPROACH DRONE:Moving Forward");
      ros::param::set("rover0/moveforward","process");

      approach_goal = rov + rov_b+ tf::Vector3(1.0,0,0).rotate(tf::Vector3(0,0,1),tf::getYaw(rover_pose.orientation));
      goal.goal_pose.position.x = approach_goal.getX();
      goal.goal_pose.position.y = approach_goal.getY();
      goal.goal_pose.orientation.w = 1.0;
      time_0 = ros::Time::now();
      ...
      {
        ROS_ERROR("APPROACH DRONE:Fuck something went wrong, MoveForward failed");
        ROS_INFO("waiting time: %f seconds",(ros::Time::now()-time_0).toSec());
        ac.cancelAllGoals();
        S = mission_failed;
      }
    ...
    case MoveForwardOA:
      ...
      ROS_INFO("APPROACH DRONE:Moving Forward with obstacle avoidance");
      ros::param::set("rover0/moveforwardOA","process");
      approach_goal = rov + tf::Vector3(1.0,0,0).rotate(tf::Vector3(0,0,1),tf::getYaw(rover_pose.orientation));
      goalOA.goal_pose.position.x = approach_goal.getX();
      goalOA.goal_pose.position.y = approach_goal.getY();
      ...
      {
        ROS_ERROR("APPROACH DRONE:Fuck something went wrong, MoveForward OA failed");
        ROS_INFO("waiting time: %f seconds",(ros::Time::now()-time_0).toSec());
        acOA.cancelAllGoals();
        S = mission_failed;
      }
